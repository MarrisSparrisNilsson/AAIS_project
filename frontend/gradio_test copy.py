...
mlflow_tracking_uri = (
    os.environ.get("MLFLOW_TRACKING_URI") or f"sqlite:///{os.path.abspath('mlflow.db')}"
)
...
def get_approved_list():
    ...
    return "\n".join(
        [f"Invoice {i+1}: {json.dumps(inv)}" for i, inv in enumerate(approved_invoices)]
    )
...
with gr.Blocks(title="Invoice Extractor") as demo:
    ...
    with gr.Tabs():
            ...
            with gr.Row():
                with gr.Column():
                    image_input = gr.Files(
                        file_types=["image"], label="Upload Invoice Image"
                    )
                ...
                with gr.Column():
                    prediction_display = gr.Textbox(
                        label="Extracted Information", lines=6, interactive=False
                    )
        ...
        with gr.Tab("Approved Invoices"):
            approved_list_display = gr.Textbox(
                label="List of Approved Invoices", lines=15, interactive=False
            )
